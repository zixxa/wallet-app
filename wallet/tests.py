...
from wallet.models import Wallet, Profile, Currency, Transaction, DEFAULT_CURRENCY, convert_to_currency, \
    deconvert_to_currency
...
class TestWalletAPI(TestCase):
    ...
    def setUp(self):
        ...
        self.user_1 = User.objects.create_user(username=user_data_1['username'], password=user_data_1['password'])
        self.profile_1 = Profile.objects.create(user=self.user_1, name=user_data_1['profile_name'])
        self.wallet_1 = Wallet.objects.create(name=user_data_1['wallet_name'], score=user_data_1['wallet_score'],
                                              profile=self.profile_1, currency=self.currency_rub)

        self.user_2 = User.objects.create_user(username=user_data_2['username'], password=user_data_2['password'])
        self.profile_2 = Profile.objects.create(user=self.user_2, name=user_data_2['profile_name'])
        self.wallet_2 = Wallet.objects.create(name=user_data_2['wallet_name'], score=user_data_2['wallet_score'],
                                              profile=self.profile_2, currency=self.currency_rub)

        self.user_3 = User.objects.create_user(username=user_data_3['username'], password=user_data_3['password'])
        self.profile_3 = Profile.objects.create(user=self.user_3, name=user_data_3['profile_name'])
        self.wallet_3_1 = Wallet.objects.create(name=user_data_3['wallet_name_1'], score=user_data_3['wallet_score_1'],
                                                profile=self.profile_3, currency=self.currency_rub)
        self.wallet_3_2 = Wallet.objects.create(name=user_data_3['wallet_name_2'], score=user_data_3['wallet_score_2'],
                                                profile=self.profile_3, currency=self.currency_usd)
    ...
    def test_post_transaction(self):
        ...
        receiver_wallet = Wallet.objects.get(name=self.transaction["wallet_receiver_name"])
        self.assertEqual(sender_wallet.score, self.user_data_1["wallet_score"] - payment)
        self.assertEqual(receiver_wallet.score, self.user_data_2["wallet_score"] + payment)
        transaction_1 = Transaction.objects.get(wallet=sender_wallet)
        transaction_2 = Transaction.objects.get(wallet=receiver_wallet)
        self.assertEqual(transaction_1.description,
                         f"-{'{:.2f}'.format(payment)} {self.transaction['currency']} - Счет: {sender_wallet.score} {DEFAULT_CURRENCY}")
        self.assertEqual(transaction_2.description,
    ...
    def test_post_transaction_in_another_currency_wallet(self):
        ...
        receiver_wallet = Wallet.objects.get(name=transaction_data["wallet_receiver_name"])
        self.assertEqual(sender_wallet.score, Decimal(str(self.user_data_1["wallet_score"])) - converted_payment)
        self.assertEqual(receiver_wallet.score, Decimal(str(self.user_data_3["wallet_score_2"])) + converted_payment)
        transaction_1 = Transaction.objects.get(wallet=sender_wallet)
        transaction_2 = Transaction.objects.get(wallet=receiver_wallet)
        self.assertEqual(transaction_1.description,
    ...
    def test_post_payment_validate_another_currency(self):
        ...
        converted_payment = convert_to_currency(payment, transaction_data["currency"])
        self.assertEqual(sender_wallet.score, Decimal(str(self.user_data_1["wallet_score"])) - converted_payment)
        self.assertEqual(receiver_wallet.score, Decimal(str(self.user_data_2["wallet_score"])) + converted_payment)
        transaction_1 = Transaction.objects.get(wallet=sender_wallet)
        transaction_2 = Transaction.objects.get(wallet=receiver_wallet)
        self.assertEqual(transaction_1.description,
    ...
    def test_post_payment_validate_balance_error(self):
        ...
        transaction_1 = Transaction.objects.get(wallet=sender_wallet)
        self.assertEqual(transaction_1.description,
                         u'Транзакция завершилась ошибкой "Недостаточно средств для перевода" - Счет: 300.00 RUB')
    ...
    def test_get_many_wallets(self):
        ...
        self.assertEqual(response.json(), {wallet.name: [float(convert_to_currency(wallet.score,wallet.currency.name)),wallet.currency.name]
