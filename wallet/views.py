...
class WalletAPI(APIView):
    ...
    def get(self, request, format=None):
        ...
        content = {wallet.name:(convert_to_currency(wallet.score, wallet.currency.name), wallet.currency.name) for wallet in wallets}
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            sender = Wallet.objects.get(profile__user_id=request.user.id, name=data["wallet_sender_name"])
            payment = Decimal(data["payment"], 2)

            if payment > convert_to_currency(sender.score, data["currency"]):
                sender.create_error_transaction(error_description="Недостаточно средств для перевода")
                return Response('Недостаточно средств для перевода', status=status.HTTP_400_BAD_REQUEST)

            receiver = Wallet.objects.get(name=data["wallet_receiver_name"])
            sender.create_transaction(payment=payment, currency=data["currency"], is_sender=True)
            receiver.create_transaction(payment=payment, currency=data["currency"], is_sender=False)
            content = {
                'user_sender': Profile.objects.get(user_id=request.user.id).user.username,
